...
from app.api.question import get_from_db_question_by_id, get_unique_jservice_questions
from app.core.db import get_async_session
from app.crud.question import question_crud
from app.models import Question
from app.schemas import QuestionDBShema, ManyQuestionParseShema
...
@router.get(
    '/',
    response_model=List[QuestionDBShema],
...
async def create_many_unique_questions(
    many_unique_questions: ManyQuestionParseShema = Depends(get_unique_jservice_questions),
    session: AsyncSession = Depends(get_async_session)
):
    """Добавление номенклатуры товара."""
    try:
        async_calls = [get_wb_product_info_by_id(id) for id in product_ids]
        parse_products: List[QuestionParseShema] = await asyncio.gather(*async_calls)
        if parse_products:
            success_product = map(
                lambda product: Question(**product),
                parse_products
            )
